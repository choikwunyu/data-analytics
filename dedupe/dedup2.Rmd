...
cols <- c("deviceId","id","uploadId","type")
patient <- read_csv("/Users/BlauBear/Documents/Tidepool/data-analytics/dedupe/data0ab293f2e1cc4141cd10bfbd5d51664992b1c63c79ac45e548417154e4ecde43.csv")
...
patient$uploadId <- as.factor(patient$uploadId)
```
...
for (i in 1:length(unique_ids)) {
  temp <- subset(patient_cbg, uploadId == unique_ids[i])
  # uploadsets[[i]] <- temp
  assign(unique_ids[i], temp)
}
...
```{r}
comparision_funct <- function(df1, df2){
  values <- StringConvert(df1$mgd,df2$mgd,3 ) #http://www.cs.ucr.edu/~eamonn/SAX.pdf alphabet size: 5-8 works better. ##3 worked better for this run
...
```{r}
# comparision_funct(data.frame(uploadsets[[1]]),data.frame(uploadsets[[2]]))
comparision_funct(upid_066b389ed302,upid_01768d2f5d30)
```
